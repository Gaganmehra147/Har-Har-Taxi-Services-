...
import { Booking, CarItem, BookingStatus, PricingModel } from "@/data/db/types";
...
export default function AdminPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (cData.success) setCars(cData.cars || []);
    } catch (err) {
  ...
  useEffect(() => {
    fetchData();
    // Auto-poll every 12 seconds to detect new bookings in real time
  ...
  // Toggle Car Active
  const handleToggleCar = async (id: string) => {
    try {
      const res = await fetch(`/api/cars/${id}`, { method: "PATCH" });
      const data = await res.json();
      if (data.success) {
        setCars(prev => prev.map(c => c.id === id ? { ...c, active: !c.active } : c));
      }
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3.5 sm:gap-5">
          <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 sm:p-5 shadow-sm">
            ...
            <span className="text-[10px] text-zinc-400 mt-1 block">Lifetime website bookings</span>
          </div>
            ...
            {filteredBookings.length === 0 ? (
              <div className="bg-white dark:bg-zinc-900 border border-dashed border-zinc-300 dark:border-zinc-700 rounded-2xl p-12 text-center">
                ...
                <p className="text-xs text-zinc-500 mt-1">
                  Jab koi website par booking form bharega, wo yahan automatic real-time show hogi.
                </p>
              </div>
            ) : (
